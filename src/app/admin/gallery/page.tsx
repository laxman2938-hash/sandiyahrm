...
export default function GalleryAdmin() {
  ...
  function handleEdit(image: GalleryImage) {
    setFormData({ title: image.title, image: image.image });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setSuccess(editingId ? 'Gallery image updated!' : 'Gallery image added!');
      resetForm();
      await fetchImages();
  ...
  return (
    <div className="space-y-8">
      ...
      {/* List */}
      <div className="bg-white p-8 rounded-lg shadow-md">
        <h2 className="text-2xl font-bold mb-6 text-gray-800">Gallery Images</h2>

        {loading ? (
          <p className="text-gray-600">Loading gallery images...</p>
        ) : images.length === 0 ? (
          <p className="text-gray-600">No images found. Add one to get started!</p>
        ) : (
